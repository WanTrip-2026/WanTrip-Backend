...
const LINE_PAY_CHANNEL_SECRET: string | undefined =
...
router.post("/linepay/request", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(400)
      .json(axiosError.response?.data || { message: "Internal Server Error" });
  }
});
...
router.post("/linepay/confirm", async (req: Request, res: Response) => {
  try {
    ...
    const axiosError = error as AxiosError;
    console.error(
      "--- LINE Pay Confirm Error ---",
      axiosError.response?.data || axiosError.message,
    );
    res
      .status(400)
      .json(axiosError.response?.data || { message: "Confirm Failed" });
  }
});
